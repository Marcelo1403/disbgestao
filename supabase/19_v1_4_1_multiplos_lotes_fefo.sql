-- Disb Gestao v1.4.1 - Multiplos lotes na Contagem FEFO
-- Execute uma unica vez no SQL Editor do Supabase depois do SQL 18 da v1.4.0.

begin;

alter table public.fefo_count_items
  add column if not exists lot text not null default '';

create index if not exists idx_fefo_items_product_validity_lot
  on public.fefo_count_items(product_code, validity_date, lot);

drop function if exists public.save_fefo_item(uuid,text,date,text,integer,integer,integer,integer,uuid);
...
begin
  ...
  if v_lot='' then raise exception 'FEFO_LOTE_OBRIGATORIO'; end if;
...
where table_schema='public' and table_name='fefo_count_items' and column_name='lot';
