...
function humanGpsError(e){const code=e?.code;if(code===1)return 'permissão de localização negada. Verifique a permissão do app/APK.';if(code===2)return 'localização indisponível no aparelho.';if(code===3)return 'tempo esgotado ao obter GPS.';const msg=String(e?.message||e||'erro ao obter localização.');if(/permission/i.test(msg))return 'permissão de localização negada. Verifique a permissão do app/APK.';return msg;}
