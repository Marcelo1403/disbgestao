...
async function prepareRuntimeCache(){
  ...
  try{const reg=await navigator.serviceWorker.register('sw.js?v=1.2.0',{updateViaCache:'none'});await reg.update();}catch(e){console.warn('SW register',e);}
}
...
// V1.2.0 - Marketplace, avaria no NRI, Transferencia e histogramas -----------------
...
renderPullDashboardCore = function(){renderPullDashboardCoreV119();const rows=filteredPullDashTrips(),type=$('pullDashType')?.value||'';renderPullFinalArrivalHistogram(rows);const factoryCard=$('pullArrivalHistogram')?.closest('.pull-arrival-card');if(factoryCard)factoryCard.classList.toggle('hidden',type==='TRANSFER');};
