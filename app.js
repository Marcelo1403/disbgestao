...
async function prepareRuntimeCache(){
  ...
  try{const reg=await navigator.serviceWorker.register('sw.js?v=1.2.1',{updateViaCache:'none'});await reg.update();}catch(e){console.warn('SW register',e);}
}
...
async function startApp(){
  ...
  if(canNri()) { await loadPending(); await loadMarketplaceModule(true); await loadPullNriPending(true); }
...
async function loadNriHistory(){if(!isAdmin())return;try{let q=sb.from('nris').select('*').order('created_at',{ascending:false}).limit(2000);const {data,error}=await q;if(error)throw error;historyNris=(data||[]).map(mapNri);await loadNriDamageIndex(historyNris);renderNriHistory();}catch(e){toast(humanError(e),'error');}}
...
async function openPullTripDetail(id,live=false){
  try{
    ...
    const actions=[];
    if(!transfer&&!live&&t.next_started_at)actions.push({label:'Ajustar TMA Revenda',class:'secondary',onClick:()=>{closeModal();openPullTmaAdjust(t.id);}});
...
async function loadPullDashboard(silent=false){
  ...
  try{
    await loadPullReferenceData();
    const [trips,market]=await Promise.all([sb.from('pull_trips').select('*').neq('status','CANCELLED').order('started_at'),sb.from('marketplace_receipts').select('*').not('ended_at','is',null).order('started_at')]);
...
function resetNriDamageEditor(){nriDamageMode=false;nriDamagePhotos=[];if($('nriDamagePallets'))$('nriDamagePallets').value=1;if($('nriDamageReason'))$('nriDamageReason').value='';setNriDamageMode(false);}
...
addNriDraftItem = function(){
  const code=normalizeCode($('nriCodigo').value),p=productsByCode.get(code),sem=$('nriSemValidade').checked,validity=sem?null:parseShortDate($('nriValidade').value),lot=$('nriLote').value.trim().toUpperCase(),qty=num($('nriQuantidade').value),pallets=Math.trunc(num($('nriPaletes').value));
  if(!p)return toast('Informe um código de produto válido.','error');if(!sem&&!validity)return toast('Informe a validade completa no formato dd/mm/aa ou selecione Sem Validade.','error');if(!lot)return toast('Informe o lote.','error');if(qty<=0)return toast('Informe a quantidade.','error');if(pallets<1)return toast('Informe a quantidade de paletes.','error');
  const damagedPallets=nriDamageMode?Math.trunc(num($('nriDamagePallets').value)):0,reason=nriDamageMode?$('nriDamageReason').value.trim():'';
  ...
  const item={id:nriEditingId||uuid(),product_code:p.code,product_name:p.name,validity_date:validity,lot,quantity:qty,pallets,block_date:validity?addDaysIso(validity,-30):null,pallet_damaged:nriDamageMode,damaged_pallets:damagedPallets,damage_reason:reason,damagePhotos:[...nriDamagePhotos]};
  const idx=nriDraftItems.findIndex(x=>x.id===item.id);if(idx>=0)nriDraftItems[idx]=item;else nriDraftItems.push(item);renderNriDraftItems();clearNriItemEditor();
};
renderNriDraftItems = function(){
  const total=nriDraftItems.reduce((s,x)=>s+x.pallets,0);$('nriItemCounter').textContent=`${nriDraftItems.length} item(ns) • ${total} NRI(s)`;$('btnCadastrarCarreta').textContent=total?`Cadastrar carreta (${total} NRIs)`:'Cadastrar carreta';const el=$('nriItemList');if(!nriDraftItems.length){el.className='item-list empty-state';el.textContent='Nenhum produto adicionado.';return;}el.className='item-list';el.innerHTML=nriDraftItems.map(x=>`<div class="item-row" data-id="${x.id}"><div class="info"><small>Produto</small><strong>${esc(x.product_code)} • ${esc(x.product_name)}</strong></div><div class="info"><small>Validade</small><strong>${x.validity_date?formatShortDate(x.validity_date):'Sem Validade'}</strong></div><div class="info"><small>Lote</small><strong>${esc(x.lot)}</strong></div><div class="info"><small>Quantidade</small><strong>${fmtNum(x.quantity)}</strong></div><div class="info"><small>Paletes / NRIs</small><strong>${x.pallets}</strong></div><div class="info"><small>Palete avariado</small><strong>${x.pallet_damaged?`Sim • ${x.damaged_pallets} • ${esc(x.damage_reason)} • ${(x.damagePhotos||[]).length} foto(s)`:'Não'}</strong></div><div class="mini-actions"><button class="mini-btn" data-act="edit">Editar</button><button class="mini-btn danger" data-act="del">Excluir</button></div></div>`).join('');
};
onNriDraftListClick = function(e){const b=e.target.closest('button[data-act]');if(!b)return;const row=b.closest('[data-id]'),item=nriDraftItems.find(x=>x.id===row.dataset.id);if(!item)return;if(b.dataset.act==='del'){nriDraftItems=nriDraftItems.filter(x=>x.id!==item.id);renderNriDraftItems();if(nriEditingId===item.id)clearNriItemEditor();return;}nriEditingId=item.id;$('nriCodigo').value=item.product_code;$('nriSemValidade').checked=!item.validity_date;$('nriValidade').value=formatShortDate(item.validity_date);$('nriLote').value=item.lot;$('nriQuantidade').value=item.quantity;$('nriPaletes').value=item.pallets;$('nriBloqueio').value=item.block_date?formatShortDate(item.block_date):'--';nriDamagePhotos=[...(item.damagePhotos||[])];$('nriDamagePallets').value=item.damaged_pallets||1;$('nriDamageReason').value=item.damage_reason||'';setNriDamageMode(!!item.pallet_damaged);updateNriValidityMode();onProductCode();$('btnAdicionarNriItem').textContent='Salvar alteração';$('btnCancelarNriItem').classList.remove('hidden');};
...
function marketplaceReceiptRowsForDashboard(){const y=Number($('pullDashYear')?.value||new Date().getFullYear()),m=Number($('pullDashMonth')?.value||0);return marketplaceDashboardReceipts.filter(r=>{const d=new Date(r.started_at);return d.getFullYear()===y&&(!m||d.getMonth()+1===m)&&r.ended_at;});}
function renderMarketplaceReceiptHistogram(){
  if(!$('marketReceiptHistogram'))return;
  const rows=marketplaceReceiptRowsForDashboard().filter(r=>Number(r.duration_seconds)>=0),secs=rows.map(r=>Number(r.duration_seconds||0));
  const avg=secs.length?secs.reduce((a,b)=>a+b,0)/secs.length:null,med=medianNumber(secs),maxSec=secs.length?Math.max(...secs):null;
  $('marketReceiptCount').textContent=rows.length;$('marketReceiptAvg').textContent=avg==null?'—':fmtDurationSeconds(Math.round(avg));$('marketReceiptMedian').textContent=med==null?'—':fmtDurationSeconds(Math.round(med));$('marketReceiptMax').textContent=maxSec==null?'—':fmtDurationSeconds(maxSec);
  const step=30,bins=[];for(let start=0;start<360;start+=step)bins.push({start,end:start+step,count:0,label:`${start}–${start+step}m`});bins.push({start:360,end:Infinity,count:0,label:'≥360m'});
  rows.forEach(r=>{const min=Number(r.duration_seconds||0)/60;const b=min>=360?bins[bins.length-1]:bins[Math.min(Math.floor(min/step),bins.length-2)];b.count++;});
  const top=Math.max(1,...bins.map(b=>b.count));$('marketReceiptHistogram').innerHTML=bins.map(b=>`<div class="pull-hist-bin" title="${esc(b.label)} • ${b.count} recebimento${b.count===1?'':'s'}"><strong>${b.count||''}</strong><div><i style="height:${b.count?Math.max(7,b.count/top*100):0}%"></i></div><span>${esc(b.label)}</span></div>`).join('');
  const groups=new Map();rows.forEach(r=>{const key=`${r.unit}|${r.supplier_name}`;if(!groups.has(key))groups.set(key,{unit:r.unit,supplier:r.supplier_name,secs:[]});groups.get(key).secs.push(Number(r.duration_seconds||0));});
  const rs=[...groups.values()].map(g=>({unit:g.unit,supplier:g.supplier,n:g.secs.length,avg:g.secs.reduce((a,b)=>a+b,0)/g.secs.length,median:medianNumber(g.secs),min:Math.min(...g.secs),max:Math.max(...g.secs)})).sort((a,b)=>b.n-a.n||a.supplier.localeCompare(b.supplier,'pt-BR'));
  $('tbodyMarketplaceReceiptHistogram').innerHTML=rs.length?rs.map(r=>`<tr><td><strong>${esc(r.supplier)}</strong></td><td>${esc(r.unit)}</td><td>${r.n}</td><td>${fmtDurationSeconds(Math.round(r.avg))}</td><td>${fmtDurationSeconds(Math.round(r.median))}</td><td>${fmtDurationSeconds(r.min)}</td><td>${fmtDurationSeconds(r.max)}</td></tr>`).join(''):'<tr><td colspan="7">Sem recebimentos Marketplace finalizados no período.</td></tr>';
}
