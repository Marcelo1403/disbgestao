...
function bindBaseEvents(){
  ...
  $('fefoLote')?.addEventListener('input',enforceLotInput);
  $('fefoLote')?.addEventListener('keydown',e=>onMultiLotKeydown('fefo',e));
  $('btnFefoAddLot')?.addEventListener('click',()=>addMultiLot('fefo'));
  $('fefoLotList')?.addEventListener('click',e=>onMultiLotListClick('fefo',e));
...
async function finalizeSalesDamage(status){
  const ids=selectedSalesReviewIds('final');if(!ids.length)return toast('Selecione ao menos um produto em análise.','error');const note=String($('salesDamageDecisionJustification')?.value||'').trim();if(!note)return toast('Informe a justificativa da decisão final.','error');
