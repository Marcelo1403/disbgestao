...
if (fs.existsSync(gradleGroovy)) {
  ...
  g = g.replace(/versionCode\s+\d+/, 'versionCode 122');
  g = g.replace(/versionName\s+["'][^"']+["']/, 'versionName "1.2.2"');
  fs.writeFileSync(gradleGroovy, g, 'utf8');
  console.log('Versao Android OK: versionCode 122 / versionName 1.2.2.');
} else if (fs.existsSync(gradleKts)) {
  let g = fs.readFileSync(gradleKts, 'utf8');
  g = g.replace(/versionCode\s*=\s*\d+/, 'versionCode = 122');
  g = g.replace(/versionName\s*=\s*["'][^"']+["']/, 'versionName = "1.2.2"');
